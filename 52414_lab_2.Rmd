...
reg_val_pot <- lm(Potential ~ Value, data = filter_2C)
ggplot(data = filter_2C, aes(x = Potential, y = Value)) +  
  geom_point()  + geom_text(aes(label=Name), size = 3, hjust='bottom', nudge_x = 0.05, col="red") + geom_smooth(method="lm", se=TRUE, fullrange=FALSE, level=0.95) + 
  theme_gray() + theme(legend.position = c(0.94, 0.9)) +
  ggtitle('Ratings of Value as a function of Potential')
new_table2c <- select(filter_2C,Name,Value,Potential)
reg_2C <- lm(data=new_table2c,Potential ~ Value)
resid <- resid(reg_2C) #List of residuals
new_table2c$resid <- resid
...
corrplot(cor_met_3B, method="pie",tl.cex =0.5,col=c("black", "white"),
         bg="lightblue")
...
PLEASE ADD YOUR SOLUTION BELOW, WITH A CLEAR SEPARATION BETWEEN THE PARTS!
Q4B
He dot clean the data he still have empty thing
