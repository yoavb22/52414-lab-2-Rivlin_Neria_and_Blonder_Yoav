...
filter_2C$reg_val_pot <- predict(lm(Potential ~ Value, data = filter_2C))
ggplot(data = filter_2C, aes(x = Potential, y = Value)) +  
  geom_point() + 
  geom_line(aes(y = reg_val_pot)) + 
  geom_text(aes(label=Name), size = 3, hjust='bottom', nudge_x = 0.05 ) +
...
Q3D
```{r}
group_2A <- group_by(fifa_players_attribures,Age)
summ_2B <- summarize(group_2A, ave_pot=mean(Potential))
p_2A <- ggplot(summ_2B, aes(x = Age, y = ave_pot))+geom_point(data = summ_2B, color = "red") + labs(x="Age", y="ave_pot")
p_2A
