...
Q2C
```{r}
filter_2C <- filter(fifa,Age < 21)
filter_2C1 <- filter(fifa,Potential > 70)
p_2C <- ggplot(filter_2C1, aes(x = Potential, y = Value))+geom_point(data = filter_2C, color = "red") + labs(x="Potential", y="Value")+ geom_smooth(method = "lm", se = FALSE)
x = lm(filter_2C1$Potential~filter_2C1$Value)
summary(x)
print(p_2C)
group_2B <- group_by(filter_2B,Age)
summ_2B <- summarize(group_2B, ave_def_pot_over=mean(Overall - Potential))
